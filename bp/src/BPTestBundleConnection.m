...
- (void)startVideoRecordingForTestClass:(NSString *)testClass method:(NSString *)method
{
    ...
    NSString *command = [NSString stringWithFormat:@"xcrun simctl io %@ recordVideo --codec=h264 --force %@", [self.simulator UDID], videoFileName];
    self.recordVideoPipe = [[NSPipe alloc] init];
    NSTask *task = [BPUtils buildShellTaskForCommand:command withPipe:self.recordVideoPipe];
...
- (void)stopVideoRecording:(BOOL)forced
{
    ...
    NSFileHandle *fh = self.recordVideoPipe.fileHandleForReading;
    ...
    if (result) {
        [BPUtils printInfo:INFO withString:@"Video task output: %@", result];
    }
    self.recordVideoPipe = nil;
